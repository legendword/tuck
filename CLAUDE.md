...
`add`, `restore`, `list`, `status`, `verify`, `config` — each in its own file under `src/commands/`. Shared helpers in `commands/mod.rs` include `CliProgress` (indicatif wrapper implementing `Progress` trait) and `check_pending` (detects interrupted operations).
